...
namespace Bearchop.Areas.NCAAF.Web.jqGrid
{
    public static class JqGridExtensions
    {
        ...
        {
            return ToJqGridData(baseList, currentPage, rowsPerPage, orderBy, orderByReverse, searchQuery, searchColumns, null);
        }
        ...
        private static IQueryable<T> ListAddSearchQuery<T>(IQueryable<T> baseList, string searchQuery, IEnumerable<string> searchColumns)
        {
            ...
            const string strpredicateFormat = "{0}.ToString().ToUpper().Contains(@0)";
            ...
            var filteredList = baseList.Where(searchExpression.ToString(), searchQuery.ToUpper());
            return filteredList;
        }
    }
}
